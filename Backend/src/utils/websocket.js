...
export function setupWebSocket(io) {
  ...
  function processOptionTick(tick, tokenMetaMap) {
    try {
      ...
      else if (meta.symbol.startsWith("BANKNIFTY"))
        spotPrice = bankniftySpotPrice;
      ...
      const bestBid = tick.best_5_buy_data?.[0]?.price
        ? parseFloat(tick.best_5_buy_data[0].price) / 100
        : 0;
      const bestAsk = tick.best_5_sell_data?.[0]?.price
        ? parseFloat(tick.best_5_sell_data[0].price) / 100
        : 0;
      let marketPrice = parseFloat(tick.last_traded_price) / 100;
      if (
        bestBid > 0 &&
        bestAsk > 0 &&
        (bestAsk - bestBid) / marketPrice < 0.1
      ) {
        // 10% spread threshold
